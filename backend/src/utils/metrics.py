...
def supply_disruption_index(
    ...
    Args:
        ...
        delta_p_freight:Freight cost proxy (BOAT ETF) deviation from mean, [0, 1].
        w1, w2, w3, w4: Configurable weights; must sum to 1.0.
...
def normalise_freight_delta(
    ...
    Args:
        current_freight: Latest freight proxy price.
        rolling_mean:    Rolling mean freight price.
        rolling_std:     Rolling std deviation. If 0, returns 0.5 (neutral).
        clip_sigmas:     Number of standard deviations to clip at.
    ...
    if rolling_std == 0:
        return 0.5
    z = (current_freight - rolling_mean) / rolling_std
    clipped = max(-clip_sigmas, min(clip_sigmas, z))
    return round((clipped + clip_sigmas) / (2 * clip_sigmas), 4)
