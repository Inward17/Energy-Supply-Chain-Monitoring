...
def fetch_historical_prices(start_date: str, end_date: str, tickers: list[str] = ENERGY_TICKERS) -> dict[str, int]:
    """
    ...
    for ticker in tickers:
        try:
            logger.info("Fetching historical %s from %s to %s...", ticker, start_date, end_date)
            # yfinance expects start and end as strings or datetime
            df = yf.download(ticker, start=start_date, end=end_date, progress=False, auto_adjust=True)
            
            if df.empty:
                logger.warning("No historical data returned for ticker: %s", ticker)
                results[ticker] = 0
                continue
            ...
            if records:
                count = upsert_price(records)
                results[ticker] = count
                logger.info("market_trawler: %s → %d historical rows stored.", ticker, count)
            else:
                results[ticker] = 0
        except Exception as exc:
            logger.error("market_trawler: failed to process historical %s — %s", ticker, exc)
            results[ticker] = 0
