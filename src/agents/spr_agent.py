...
def calculate_spr_impact(
    ...
    gdp_hit  = round(gap_days * _GDP_LOSS_PER_DAY_PCT, 3)
    infl_hit = round(gap_days * _INFL_RISE_PER_DAY_PCT, 3)
    gdp_hit_adj  = round(adjusted_gap * _GDP_LOSS_PER_DAY_PCT, 3)
    ...
    return {
        ...
        "macro_gdp_impact_pct":     f"-{gdp_hit:.3f}%",
        "macro_gdp_impact_usd":     f"~${gdp_usd_loss:,.0f}M loss",
        "macro_infl_impact":        f"+{infl_hit:.3f}%",
